	...
	<section class="page">
		...
		<div class="bread-crumb">
			<ul>
				...
				<li><a href="${path0 }/EditData.do?bno=${data.bno}">수정</a></li>
			</ul>
		</div>
		<hr>
		<form action="${path0 }/EditDataPro.do" method="post"  >
			<table id="table">
				<tr class="tr-bno">
						<td>
							<input type="text" name="bno" id="bno" class="form-control" value="${data.bno}" >
						</td>
					</tr>
				<tr>
				<tr>
					<td><input type="text" name="title" id="title" maxlength="100" required placeholder="제목" value="${data.title }" /></td>
				</tr>
				<tr>
					<td><textarea  name="content" id="content" required placeholder="내용을 입력하세요">${data.content }</textarea></td>
				</tr>
				<tr>
					<td class="td-filename">
							<div class="btn-upload" >사진 첨부</div>
							<input type="file" name="filename" id="filename" accept=".jpg, .png, .jpeg, .svg, .PNG, .JPG, .JPEG, .SVG, .hwp, .pdf, .xls, .xlsx, .ppt, .pptx, .doc, .docx, .zip, .txt" 
								value="${path0 }/upload/data/${data.filename}" disabled/>
							<p>${data.filename}</p>
						</td>
				</tr>
				<tr>
						<td class="td-submit">
							<div class="btn-group">
								<a class="btn-back" href="${path0 }/GetData2.do?bno=${data.bno}">취소</a>
